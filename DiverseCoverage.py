...
class Node:
    def __init__(self,name,feature1,feature2,feature3):
        ...
        self.virtual = False ##This is an added fetaure to know whether a node is vrtual or not 
        self._visited = False
        self._covered = False
...
def permute(vnodeList,l,r,combinationList):
    ...
    if (l==r):
        ...
        for i in range(l,r+1):
            ...
            permute(vnodeList,l+1,r,combinationList) ##recursive
            vnodeList[l],vnodeList[i] = vnodeList[i],vnodeList[l]
...
def createVirtualGraph(vGraph):
    ...
    v1 = vGraph.createVirutalNode('v1', True, False, False) ##feature 1 virtual node
    v2 = vGraph.createVirutalNode('v2', False, True, False) ##feature 2 virtual node
    v3 = vGraph.createVirutalNode('v3', False, False, True) ##feature 3 virtual node
...
if __name__ == "__main__":
    ...
    start = time.time() ##track of how much tym does the process takes
    ...
    for i in range(20):
        ...
        vGraph = GraphClass("virtual Graph") ##Object of graphClass
        vGraph.G = Graph.G.copy() ##returns graph copy--??? Which graph copy??
        vNodeList = createVirtualGraph(vGraph) ##This has all three virtual nodes.
        if not lessNodes and not moreNodes: ##equal 
            combinationList = []
            permute(vNodeList, 0, 2, combinationList) ##We decide which to pick first node then second and then third !
